  ...
  Future<CacheEntry?> _getDiskCache(String key) async {
    ...
    try {
      ...
      if (fileSize > _config.diskIOBufferSize) {
        ...
        final bytes = await stream.fold<List<int>>(<int>[], (previous, element) => previous..addAll(element));
      ...
      return CacheEntry(
        ...
        data: entryData ?? {},
        expiryTime: DateTime.fromMillisecondsSinceEpoch(cacheData['expiryTime'] as int? ?? 0),
        priority: CachePriority.values[cacheData['priority'] as int? ?? 0],
        size: cacheData['size'] as int? ?? 0,
        accessCount: cacheData['accessCount'] as int? ?? 0,
        lastAccessed: DateTime.fromMillisecondsSinceEpoch(cacheData['lastAccessed'] as int? ?? 0),
        tags: tags,
